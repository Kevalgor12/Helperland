<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Home</title>
	<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <link rel="stylesheet" type="text/css" href="../assets/css/style.css">
</head>
<body>
  <div class="scroll-up-btn top-arrow">
    <i class="fas fa-chevron-up"></i>
  </div>

  <div class="scroll-up-btn message-box">
    <img src="../assets/images/layer-598.png" alt="message" class="img-fluid">
  </div>

  <?php
    include("header_1.php");
  ?>

  <section class="home" id="home">
    <div class="max-width">
      <div class="home-content">
        <div class="text-1">Lorem ipsum text</div>
        <div class="text-2"><img src="../assets/images/ic-check.png"> Lorem ipsum dolor sit amet, consectetur adipisicing</div>
        <div class="text-2"><img src="../assets/images/ic-check.png"> Lorem ipsum dolor sit amet, consectetur adipisicing</div>
        <div class="text-2"><img src="../assets/images/ic-check.png"> Lorem ipsum dolor sit amet, consectetur adipisicing</div>
      </div>
      <div class="button">
        <button class="button-cleaner mx-auto">Let's Book a Cleaner</button>
      </div>
      <div class="pay-service">
        <div class="pay-content">
          <div class="pay-image">
            <img src="../assets/images/step-1.png">
          </div>
          <br>
          <span>Enter your Postcode</span>
        </div>
        <div class="pay-content">
          <div class="pay-arrow">
            <img src="../assets/images/step-arrow-1.png">
          </div>
        </div>
        <div class="pay-content">
          <div class="pay-image">
            <img src="../assets/images/step-2.png">
          </div>
          <br>
          <span>Select your plan</span>
        </div>
        <div class="pay-content">
          <div class="pay-arrow">
            <img src="../assets/images/step-arrow-1-copy.png">
          </div>
        </div>
        <div class="pay-content">
          <div class="pay-image">
            <img src="../assets/images/step-3.png">
          </div>
          <br>
          <span>Pay securely online</span>
        </div>
        <div class="pay-content">
          <div class="pay-arrow">
            <img src="../assets/images/step-arrow-1.png">
          </div>
        </div>
        <div class="pay-content">
          <div class="pay-image">
            <img src="../assets/images/step-4.png">
          </div>
          <br>
          <span>Enjoy amazing service</span>
        </div>
      </div>
      <div class="arrow">
        <img src="../assets/images/group-18_5.png">
      </div>
    </div>
  </section>

  <section class="why-helperland" id="why-helperland">
    <div class="max-width">
      <div class="helperland-content">
        <b class="text-1">Why Helperland</b>
      </div>
      <div class="helperland-images row justify-content-center me-0 ms-0">
        <div class="col-sm-12 col-md-6 col-lg-4 mb-5">
          <img class="img-fluid" src="../assets/images/helper-img-2.png">
          <h3>Experience & Vetted <br> Professionals</h3>
          <span>
            dominate the industry in scale and scope <br> with an adaptable, extensive network that <br> consistently delivers exceptional results.
          </span>
        </div>
        <div class="col-sm-12 col-md-6 col-lg-4 mb-5">
          <img class="img-fluid" src="../assets/images/helper-img-3.png"><br>
          <h3>Secure Online Payment</h3>
          <span>
            Payment is processed securely online. <br> Customers pay safely online and <br> manage the booking.
          </span>
        </div>
        <div class="col-sm-12 col-md-6 col-lg-4 mb-5">
          <img class="img-fluid" src="../assets/images/helper-img-4.png"><br>
          <h3>Dedicated Customer Service</h3>
          <span>
            to our customers and are guided in all we do <br> by their needs. The team is always happy to <br> support you and offer all the information.
          </span>
        </div>
      </div>
    </div>
  </section>

  <section class="blog" id="blog">
    <img class="blog-img-left" src="../assets/images/blog-left-bg.png" alt="">
    <img class="blog-img-right" src="../assets/images/blog-right-bg.png" alt="">
    <div class="max-width">
      <div class="row">
        <div class="blog-content col-lg-7">
          <p class="lorem"><b>Lorem ipsum dolor sit amet, consectetur</b></p>
          <p>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec nisi sapien, suscipit ut accumsan vitae, pulvinar ac libero.
          </p> 
          <p>
            Aliquam erat volutpat. Nullam quis ex odio. Nam bibendum cursus purus, vel efficitur urna finibus vitae. Nullam finibus aliquet pharetra. Morbi in sem dolor. Integer pretium hendrerit ante quis vehicula.
          </p>
          <p>
            Mauris consequat ornare enim, sed lobortis quam ultrices sed.
          </p>
        </div>
        <div class="bloggroup-img col-lg-5">
          <img class="img-fluid" src="../assets/images/group-36.png">
        </div>
      </div>
      <div class="our-blog">
        <p><b>Our Blog</b></p>
      </div>
      <div class="blog-images">
        <div class="blog-img-text">
          <img class="blog-img img-fluid" src="../assets/images/group-28.png">
          <p class="blog-lorem"><b>Lorem ipsum dolor sit amet</b></p>
          <p class="blog-date">January 28, 2019</p>
          <p class="blog-text">Lorem ipsum dolor sit amet, consectetur <br> adipiscing elit. Sed fermentum metus <br> pulvinar aliquet.</p>
          <p class="blog-readpost">Read the Post <img class="blog-arrow" src="../assets/images/shape-2-copy-3.png"></p>
        </div>
        <div class="blog-img-text">
          <img class="blog-img img-fluid" src="../assets/images/group-29.png">
          <p class="blog-lorem"><b>Lorem ipsum dolor sit amet</b></p>
          <p class="blog-date">January 28, 2019</p>
          <p class="blog-text">Lorem ipsum dolor sit amet, consectetur <br> adipiscing elit. Sed fermentum metus <br> pulvinar aliquet.</p>
          <p class="blog-readpost">Read the Post <img class="blog-arrow" src="../assets/images/shape-2-copy-3.png"></p>
        </div>
        <div class="blog-img-text">
          <img class="blog-img img-fluid" src="../assets/images/group-30.png">
          <p class="blog-lorem"><b>Lorem ipsum dolor sit amet</b></p>
          <p class="blog-date">January 28, 2019</p>
          <p class="blog-text">Lorem ipsum dolor sit amet, consectetur <br> adipiscing elit. Sed fermentum metus <br> pulvinar aliquet.</p>
          <p class="blog-readpost">Read the Post <img class="blog-arrow" src="../assets/images/shape-2-copy-3.png"></p>
        </div>
      </div>
    </div>
  </section>

  <section class="customer-say" id="customer-say">
    <div class="max-width">
      <div class="customer-content">
        <div>
          <p class="customer"><b>What Our Customers Say</b></p>
        </div>
        <div class="row row-cols-1 row-cols-sm-1 row-cols-md-2 row-cols-lg-3 g-4 justify-content-center mb-5 me-0 ms-0">
          <div class="col">
            <div class="card">
              <div class="card-body p-4">
                <div class="d-flex mb-3">
                  <div class="flex-shrink-0">
                    <img src="../assets/images/group-31.png" alt="...">
                  </div>
                  <div class="flex-grow-1 ms-3 align-self-center">
                    <h5 class="mb-0">Lary Watson</h5>
                    <small class="text-muted">Manchester</small>
                  </div>
                  <div class="flex-shrink-0">
                    <img src="../assets/images/message.png" alt="...">
                  </div>
                </div>
                <p class="card-text">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed fermentum metus pulvinar aliquet consequat. Praesent nec malesuada nibh.</p>
                <p class="card-text">Nullam et metus congue, auctor augue sit amet, consectetur tortor.</p>
                <a href="#">Read More <img src="../assets/images/shape-2.png" alt="right arrow"></a>
              </div>
            </div>
          </div>
          <div class="col">
            <div class="card">
              <div class="card-body p-4">
                <div class="d-flex mb-3">
                  <div class="flex-shrink-0">
                    <img src="../assets/images/group-31.png" alt="...">
                  </div>
                  <div class="flex-grow-1 ms-3 align-self-center">
                    <h5 class="mb-0">Lary Watson</h5>
                    <small class="text-muted">Manchester</small>
                  </div>
                  <div class="flex-shrink-0">
                    <img src="../assets/images/message.png" alt="...">
                  </div>
                </div>
                <p class="card-text">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed fermentum metus pulvinar aliquet consequat. Praesent nec malesuada nibh.</p>
                <p class="card-text">Nullam et metus congue, auctor augue sit amet, consectetur tortor.</p>
                <a href="#">Read More <img src="../assets/images/shape-2.png" alt="right arrow"></a>
              </div>
            </div>
          </div>
          <div class="col">
            <div class="card">
              <div class="card-body p-4">
                <div class="d-flex mb-3">
                  <div class="flex-shrink-0">
                    <img src="../assets/images/group-31.png" alt="...">
                  </div>
                  <div class="flex-grow-1 ms-3 align-self-center">
                    <h5 class="mb-0">Lary Watson</h5>
                    <small class="text-muted">Manchester</small>
                  </div>
                  <div class="flex-shrink-0">
                    <img src="../assets/images/message.png" alt="...">
                  </div>
                </div>
                <p class="card-text">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed fermentum metus pulvinar aliquet consequat. Praesent nec malesuada nibh.</p>
                <p class="card-text">Nullam et metus congue, auctor augue sit amet, consectetur tortor.</p>
                <a href="#">Read More <img src="../assets/images/shape-2.png" alt="right arrow"></a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div>
      <p class="newsletter">GET OUR NEWSLETTER</p>
    </div>
    <div class="text-center">
      <input type="text" name="email" placeholder="YOUR EMAIL" class="your-email mb-3">
      <button type="submit" class="get-news-submit">Submit</button>
    </div>
  </section>

<?php
  include("footer_1.php");
?>